...
from car_api.serializers import (
    CarSerializer, ReservationSerializer, CarDetailsReservationSerializer, AvailableCarSerializer)
...
def view_car_details_active_booking(request, car_pk):
    ...
    if request.method == 'GET':
        ...
        CarBookingDetails = namedtuple(
            'CarBookingDetails', ('car', 'current_active_bookings'))
        condition_1 = Q(issue_date__gte=current_date)
        condition_2 = Q(return_date__gte=current_date)

        carBookingDetails = CarBookingDetails(
            car=car,
            current_active_bookings=Reservation.objects.filter(
                car=car_pk).filter(condition_1 | condition_2),
        )
...
def view_all_cars_on_given_date(request):
    ...
    if request.method == 'GET':
        ...
        cars = Car.objects.all()
        filters = {'model': model, 'availability': availability}
